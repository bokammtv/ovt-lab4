...
function evaluate(str) {
    ...
    for (token of tokenize(str)) {
        ...
        if (isNumeric(token)) {
            ...
            switch (token) {
                case '+':
                    temp = (parseFloat(out[left])) + (parseFloat(out[right]));
                    break;
                case '-':
                    temp = (parseFloat(out[left])) - (parseFloat(out[right]));
                    break;
...
function clickHandler(event) {
    ...
    if (className == 'digits') return//чтоб не тыкали в пустые области
    if (className == 'other') return
    let value = event.target.innerHTML;
    let screen = document.querySelector('.exp');
    if (value == 'C') {//очистка экрана
        screen.innerHTML = '';
    } else if (value == '=') {//вычисления
        let RPN = compile(screen.innerHTML);
        let temp = evaluate(RPN);
        screen.innerHTML = '';
        screen.append(temp);
    } else {
        screen.append(value);//вывод значения нажатой кнопки
    }
}
